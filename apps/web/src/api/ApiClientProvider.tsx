...
export function ApiClientProvider({ children }: { children: ReactNode }) {
  ...
  const [isFethingAccessToken, setFetchingAccessToken] =
  ...
  const fetchAccessToken = useCallback(async () => {
    try {
      ...
      setError(
        "API access error:" + (e instanceof Error ? e.message : e?.toString()),
      );
    }
  ...
  return (
    <>
      {error && (
        <Snackbar type="danger" msg={error} onClose={() => setError("")} />
      )}
