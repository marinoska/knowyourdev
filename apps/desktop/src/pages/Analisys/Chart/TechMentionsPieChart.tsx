...
const LegendItems = [
    ...
    },
    {
        ...
        tooltipText: 'Technologies listed in the CV\'s skills section but not supported by references in the job descriptions.'
    },
    {
        label: 'In Jobs only',
        color: SoftGrayColor,
        tooltipText: 'Technologies mentioned in job descriptions but not currently used by the candidate.'
    },
];
...
export const TechMentionsPieChart = () => {
    ...
    const {chartData, sliceColors}
        ...
        const data: [string, (number | string), (string | typeof tooltipField)][] = [["Technology", "Percentage", tooltipField]];
        const colors: string[] = [];

        const totalTechCount = inSkillsAndJobDescription.length + inJobDescriptionOnly.length + inSkillsOnly.length;
        ...
        inSkillsAndJobDescription.forEach((tech) => addTechInDate(tech, GreenLegendColor));
