...
export async function matchTechList(
  ...
  const matches: TechCode[] = this.components.and.filter((code: TechCode) =>
    techCodeSet.has(code),
  );
  ...
  const percentMatches =
    matches.length / (this.components.or.length + this.components.and.length);
