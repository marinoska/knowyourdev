...
import { StackComponents, TechStackCategory } from "@/models/types.js";
...
const dropExistingCollection = async (name: string) => {
  ...
  if (collections.filter((coll) => coll.name === name)) {
...
const loadTechData = async (fileName: string) => {
  ...
  try {
    ...
    console.log(
      `${result.length} documents successfully inserted in Tech collection.`,
    );
  } catch (err) {
...
export const loadTechStackData = async (fileName: string): Promise<void> => {
  ...
  try {
    ...
    const formatRecord = (record: any): TechStackData => ({
      ...
      components: record.Components?.split(",").reduce(
        (acc: StackComponents, item: string) => {
      ...
      trend: record.Trend.split(/\s+/)
        .map((word: string) => word.charAt(0).toUpperCase())
        .join(""),
      popularity: record.Popularity ? record.Popularity : undefined,
      languages: record.Languages?.split(",").map(generateTechCode),
      relations: record.Relations?.split(",").map(generateTechCode),
