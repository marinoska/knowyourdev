...
export const extractTechProficiency = async (
  params: Params,
): Promise<Output["technologies"]> => {
  const { inputTechList, description } = params;

  const profExtractor = RunnableSequence.from<
    { description: string; input_tech_list: string[] },
    Output
  >([
  ...
  const proficiencyNormalizer = semanticSimilarity<ProficiencyType>([
    ...PROFICIENCY,
  ]);
  Object.keys(technologies).forEach((tech) => {
    output[tech] =
      proficiencyNormalizer(technologies[tech]) || ("" as ProficiencyType);
  });
