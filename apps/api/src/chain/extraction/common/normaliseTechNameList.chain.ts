...
export const normalizeTechList = async ({
  inputTechList,
  techDocList,
}: Params): Promise<Omit<TechnologyEntry, "proficiency">[]> => {
  ...
  const codeTechDocMap = techDocList.reduce<Record<string, TechDocument>>(
    (acc, techDoc) => ({
      ...acc,
  ...
  const techs = technologies.map((tech) => {
    const normalized = tech.normalized
      ? tech.normalized
      : techNameSemanticNormalizer(tech.original);

    const code = normalized
      ? techNameOverlapNormalizer(generateTechCode(normalized))
      : techNameOverlapNormalizer(generateTechCode(tech.original));
