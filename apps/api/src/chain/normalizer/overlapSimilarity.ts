export const overlapSimilarity =
  <T extends string>(referenceList: T[]) =>
  (value: string): T | undefined => {
    ...
    const jaccardSimilarityNGrams = (
      ...
      const getNGrams = (str: string, n: number): string[] =>
        [...str]
          .map((_, i, arr) =>
            i < arr.length - (n - 1) ? arr.slice(i, i + n).join("") : null,
          )
      ...
      const intersection = new Set(
        [...set1].filter((ngram) => set2.has(ngram)),
      );
